...
class Project extends Model {
  ...
  */
  ...
  destroyed () {
    ...
    for (let watcher = 0; watcher < this.watcherPromisesByPath.length; watcher++) {
      _ = this.watcherPromisesByPath[watcher]; watcher.dispose()
    }
    this.rootDirectories = []
    this.repositories = []
  }
  ...
  */
  ...
  addPath (projectPath, options = {}) {
    ...
    this.watcherPromisesByPath[directory.getPath()] = watchPath(directory.getPath(), {}, (events) => {
      // Stop event delivery immediately on removal of a rootDirectory, even if
      // its watcher promise has yet to resolve at the time of removal
  ...
  relativizePath (fullPath) {
    ...
    if (fullPath != null) {
      for (let rootDirectory of this.rootDirectories) {
        ...
        if (relativePath && (relativePath.length < result[1].length)) {
